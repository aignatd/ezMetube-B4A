...
Sub Process_Globals
	...
	Dim LblHome					As String = "Home"
	Dim LblBrowse				As String = "Browse"
	Dim LblLogin				As String = "Login"
	Dim lblExit					As String = "Do you want to exit ?"

	Dim icWarning				As String = "ic_action_warning"
	Dim icHome					As String = "home.png"
	Dim icBrowse				As String = "browse.png"
	Dim icLogin					As String = "login.png"
	Dim icTopHeader				As String = "topheader"
	Dim icPanelInfo				As String = "panelinfo"
	Dim icMenuPilih				As String = "menu_icon"
	Dim iccaridata				As String = "search_icon"
	Dim icPanelKiri				As String = "panelkiri"
	Dim icTutup					As String = "cancel_icon"
	Dim icPanahNext				As String = "arrownext"
	Dim icSetting				As String = "gear"

End Sub

Sub SetDirProg(FolderBaru As String)
	If File.ExternalWritable Then DirDBS = File.DirRootExternal Else DirDBS = File.DirInternal
	...
	Select Status
		Case "NextArray"
			...
			For Each colroot As Map In root
				...
				mpList.Put("key1", colLogin.Get("key1"))
				mpList.Put("key1a", colLogin.Get("key1a"))
				mpList.Put("key2a", colLogin.Get("key2a"))
				mpList.Put("value", colLogin.Get("value"))
				Next
				
				Dim FacebookList As List = colroot.Get("LoginFB")
				For Each colFacebook As Map In FacebookList
				mpList.Put("FacebookURL", colFacebook.Get("FacebookURL"))
				mpList.Put("FB_id", colFacebook.Get("FB_id"))
				mpList.Put("FB_photo", colFacebook.Get("FB_photo"))
				mpList.Put("FB_display", colFacebook.Get("FB_display"))
				mpList.Put("FB_mail", colFacebook.Get("FB_mail"))
				mpList.Put("FB_mail", colFacebook.Get("FB_mail"))
				Next
				
				Dim CategoryList As List = colroot.Get("GetListVideo")
				For Each colCategory As Map In CategoryList
				mpList.Put("ListURL", colCategory.Get("ListURL"))
				mpList.Put("List", colCategory.Get("List"))
				mpList.Put("Jumlah", colCategory.Get("Jumlah"))
				mpList.Put("Halaman", colCategory.Get("Halaman"))
				Next
